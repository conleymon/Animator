...
var ObjectAnimator = function (p = {}) {
  ...
  this.roll = function () {//self perpetuation, always delayed one iteration
    if (this.running()) {
      //timeout is faster/smoother for some reason
      /*
  ...
  this.iterate = function () {
    ...
    this.inc(this.fDest, this.src, this.fOrig, prog) //set the incremental values
  ...
  this.loadAnimation = function (p) {//src,dest,orig,duration,preInc,postInc,preAnim,postAnim , assumes p is writable
    ...
    //orig should only be submitted if src is not reflective of the current object status. It is an explicitly set starting point.
