...
function Queue(){
  ...
  this.finally=(cb)=>{callBack=cb;return this}
  ...
  var listen=()=>{
    ...
    if(!q.initiated){
      if(q.preCondition()){
        ...
        if(q.result instanceof Promise){ this.insert(q.result)}  
        if(!q.wait){
          q.result=result
        }
        q.initiated=true
...
Queue.promise=function(p={}){//starts another Queue instance and hooks up the call back 
  var{promise}=p
  if(p.constructor===Promise){promise=p}
  var task=function(p){
    promise.finally(()=>{p.done(promise)})
  }
  return Object.assign({task,promise},p)
}

Queue.promiseWrap=function(p={}){
...
Queue.fetch=function(p={}){//must include 'fetchPackage' parameter  or p will be interpreted as the fetchPackage
  var {pack}=p
  if(!pack.fetchPackage){pack={fetchPackage:p}}
  pack.wait=false
  pack.task=()=>{return fetch(pack.fetchPackage)}
  return pack
}
