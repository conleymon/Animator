...
exports.getOwnEnumerableProperties = __webpack_require__(34);

/*!
 * Checks error against a given set of criteria
 */

exports.checkError = __webpack_require__(35);
...
exports.isNaN = __webpack_require__(36);
...
var _unique_id = __webpack_require__(43);

var _animator = __webpack_require__(16);

var _queue = __webpack_require__(44);

var chai = __webpack_require__(9);
var expect = chai.expect;


describe((0, _unique_id.prefixId)('Animator functions'), function () {
    var dest,
        src,
        preAnim,
        postAnim,
        preInc,
        postInc,
        animator = new _animator.ObjectAnimator();
    ...
    it((0, _unique_id.prefixId)('animates deeply to target values'), function (done) {
        postAnim = function postAnim() {
            expect(src, "final values don't match").to.deep.equal(dest);
            done();
        };
        animator.anim({ src: src, dest: dest, postAnim: postAnim });
    });

    it((0, _unique_id.prefixId)(' executes callBacks when expected'), function (done) {
        var before = false,
            during1 = 0,
            during2 = 0,
            after = false,
            seqFuncMessage = 'functions are out of order or not called';

        preAnim = function preAnim() {
            expect([before, during1, during2, after], seqFuncMessage + ' preanim').to.deep.equal([false, 0, 0, false]);
            before = true;
        };

        preInc = function preInc() {
            //            if(during1!==during2){
            expect(during1, 'inc funcs are out of order pre').to.equal(during2);
            //            }
            during1++;
        };
        postInc = function postInc() {
            //            if(!(during1>=during2)){
        ...
        animator.anim({ src: src, dest: dest, preAnim: preAnim, preInc: preInc, postInc: postInc, postAnim: postAnim });
    });

    describe((0, _unique_id.prefixId)('animates colors rgb, hex and color names'), function () {
        var incCalled = false,
            firstRed,
            firstBlue;
        ...
        var getNumbers = function getNumbers(col) {
            var col = col.split('(')[1];
            col = col.split(',').map(function (v) {
                return Number(v);
            });
            return col;
        };

        var doTest = function doTest(srcColor, destColor, done) {
            src.color = srcColor;
        ...
        for (var r in srcRepsRed) {
            for (var b in srcRepsBlue) {
                it((0, _unique_id.prefixId)(r + ' to ' + b), function (done) {
                    doTest(srcRepsRed[r], srcRepsBlue[b], done);
                });
            }
        }
    });
    it((0, _unique_id.prefixId)('animates transforms independently, preserves src order'), function (done) {
        ...
        var animator2 = new _animator.ObjectAnimator();

        var getTask = function getTask(animator, dest) {
            return {
                task: function task(p) {
                    animator.anim({
                        src: src,
                        dest: dest,
                        synchTransInc: true,
        ...
        new _queue.Queue().all([getTask(animator, dest), getTask(animator2, dest2)]).add(function () {
            expect(src.transform).to.equal('translateX(100%) translateY(20%) rotateY(90deg)');
            done();
        }).kickStart();
    });
    it((0, _unique_id.prefixId)('animates only variables on dest'), function (done) {
    ...
    it((0, _unique_id.prefixId)('stop works'), function (done) {
        var last = src.one.two.three.val;
        preInc = function preInc() {
            last = src.one.two.three.val;
        };
        postAnim = function postAnim() {
            expect(last < 3).to.equal(true);
            done();
        };
        animator.anim({ src: src, dest: dest, preInc: preInc, postAnim: postAnim, duration: 1000 });
        setTimeout(function () {
            animator.stop();
        }, 500);
    });
...
var prefixId = exports.prefixId = function prefixId(string) {
    return id++ + '_' + string;
};
var postfixId = exports.postfixId = function postfixId(string) {
    return string + '_' + id++;
};

//instance
var GetIDGenerator = exports.GetIDGenerator = function GetIDGenerator() {
    var id = 0;
    this.getId = function () {
        return id++;
    };
    this.prefixId = function (string) {
        return id++ + '_' + string;
    };
    this.postfixId = function (string) {
        return string + '_' + id++;
    };
};
...
if (module && module.exports) {
  var ObjectAnimator = __webpack_require__(16).ObjectAnimator;
}
...
function Queue() {
  ...
  this.finally = function (cb) {
    callBack = cb;return _this;
  };

  this.initVal = function (v) {
    initialValue = v;return _this;
  };
  ...
  var getControlPackage = function getControlPackage() {
    ...
    var ret = {
      control: _this,
        ...
        queueLine[indexMatch].result = result;lastResult = result;
        if (!waitRunning) {
          return;
        }
        setTimeout(function () {
...
Queue.promise = function () {
  ...
  return Object.assign({ task: task, promise: promise }, p);
};
...
Queue.fetch = function () {
  ...
  pack.wait = false;
  pack.task = function () {
    return fetch(pack.fetchPackage);
  };
  return pack;
};
Queue.ajax = function () {
...
if (window.wait === undefined) {
  window.wait = Queue.wait;
}
if (window.animate === undefined) {
  window.animate = Queue.animate;
}
if (window.transition === undefined) {
  window.transition = Queue.transition;
}
